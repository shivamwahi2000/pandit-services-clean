...
export default function KundliAnalysisPage() {
    ...
    return (
      <div className="min-h-screen sacred-bg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-text-secondary">Loading Kundli Analysis...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen sacred-bg page-load">
        ...
        <main className="pt-8 pb-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-3xl font-bold text-primary mb-4">Kundli Analysis</h1>
            <div className="bg-surface rounded-2xl p-8 border border-line">
              <div className="text-6xl mb-4">📊</div>
              <h2 className="text-xl font-semibold text-text-primary mb-2">No Kundli Data Found</h2>
              <p className="text-text-secondary mb-6">
                Please generate a Kundli first from the Astrology & Vastu page.
              </p>
              <a 
  ...
  const tabs = [
    ...
    { id: 'nakshatra', label: 'Nakshatra Details', icon: '⭐' },
    { id: 'panchang', label: 'Panchang Details', icon: '📅' }
  ];
      ...
      <main className="pt-8 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <section className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold heading-en text-primary mb-4">
              Kundli Analysis
            </h1>
            ...
            <div className="bg-surface rounded-xl p-4 border border-line max-w-4xl mx-auto">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-sm">
                <div>
                  ...
                  <span className="text-text-primary font-semibold">{result.name || form.name}</span>
                </div>
                <div>
                  <span className="text-text-secondary block">Date</span>
                  <span className="text-text-primary font-semibold">{form.dateOfBirth}</span>
                </div>
                <div>
                  <span className="text-text-secondary block">Time</span>
                  <span className="text-text-primary font-semibold">{form.timeOfBirth}</span>
                </div>
                <div>
                  <span className="text-text-secondary block">Place</span>
                  <span className="text-text-primary font-semibold">{location?.name}</span>
                </div>
              </div>
            </div>
          </section>
            ...
            <section className="mb-12">
              ...
              {activeTab === 'chart' && data?.planets && (
                <div className="space-y-8">
                  <div className="bg-surface rounded-xl p-6 border border-line">
                    ...
                    <p className="text-text-secondary mb-6">Traditional North Indian style birth chart showing planetary positions in houses with authentic Vedic formatting</p>
                  ...
                  {typeof data === 'object' && data !== null && 'navamsa_chart' in data && (data as any).navamsa_chart && Object.keys((data as any).navamsa_chart).length > 0 && (
                    <div className="bg-surface rounded-xl p-6 border border-line">
                      ...
                      <p className="text-text-secondary mb-6">Traditional divisional chart for marriage and spiritual life analysis</p>
              ...
              {activeTab === 'planets' && data?.planets && (
                <div className="bg-surface rounded-xl border border-line overflow-hidden">
                  <div className="p-6 border-b border-line">
                    ...
                    <p className="text-text-secondary mt-2">Detailed planetary positions in your birth chart</p>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-elevations">
                        <tr>
                          <th className="px-6 py-4 text-left text-sm font-semibold text-text-primary">Planet</th>
                          <th className="px-6 py-4 text-left text-sm font-semibold text-text-primary">Sign</th>
                          <th className="px-6 py-4 text-left text-sm font-semibold text-text-primary">House</th>
                          <th className="px-6 py-4 text-left text-sm font-semibold text-text-primary">Degrees</th>
                          <th className="px-6 py-4 text-left text-sm font-semibold text-text-primary">Longitude</th>
                        </tr>
                      </thead>
                          ...
                          <tr key={planetName} className={`hover:bg-elevations/50 transition-colors ${
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-semibold text-text-primary">{planetName}</div>
                                  <div className="text-xs text-text-secondary capitalize">
                                    {planetName === 'Rahu' || planetName === 'Ketu' ? 'Shadow Planet' : 
                                     ['Sun', 'Moon'].includes(planetName) ? 'Luminary' : 'Planet'}
                                  </div>
                                </div>
                              </div>
                            </td>
                      ...
                      return (
                        <div key={houseNum} className={`rounded-lg p-4 border-2 transition-all ${
                          ...
                          <div className="flex items-center justify-between mb-3">
                            <h3 className="font-bold text-lg text-text-primary">House {houseNum}</h3>
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              hasplanets ? 'bg-primary text-white' : 'bg-line text-text-secondary'
                            }`}>
                              {houseData?.planet_count || 0} planets
                            </span>
                          </div>
                          ...
                          {hasplanets ? (
                            ...
                            <p className="text-text-secondary text-sm italic">No planets in this house</p>
                          )}
                        </div>
                      );
                    })}
                ...
                <div className="bg-surface rounded-xl p-6 border border-line">
                  ...
                  {data?.yogas && data.yogas.length > 0 ? (
                        ...
                        {data.yogas.map((yoga, index: number) => (
                          <div key={index} className={`rounded-xl p-6 border-l-4 shadow-sm hover:shadow-md transition-all ${
                            ...
                            <div className="flex justify-between items-start mb-4">
                              <h3 className="font-bold text-xl text-text-primary">{yoga.name}</h3>
                              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                                yoga.benefic !== false ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                              }`}>
                                {yoga.benefic !== false ? '✅ Beneficial' : '⚠️ Challenging'}
                              </span>
                            </div>
                            
                            <p className="text-text-secondary mb-4 leading-relaxed">{yoga.description}</p>
                            
                            {yoga.planets_involved && yoga.planets_involved.length > 0 && (
                              <div className="flex flex-wrap gap-2">
                                <span className="text-sm text-text-secondary font-medium">Planets:</span>
                    ...
                    <div className="text-center py-12">
                      <div className="text-6xl mb-4">🔮</div>
                      <h3 className="text-xl font-semibold text-text-primary mb-2">Calculating Yogas</h3>
                      <p className="text-text-secondary">Yoga analysis is being processed...</p>
                    </div>
                  )}
                </div>
              )}
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-6 border border-blue-200">
                        <h3 className="font-bold text-lg text-blue-800 mb-3">Birth Nakshatra</h3>
                        <p className="text-2xl font-bold text-primary mb-2">
                          {data?.nakshatra_details?.birth_nakshatra || 'Calculating...'}
                        </p>
                        <p className="text-sm text-blue-700">Your birth star constellation</p>
                      </div>
                      
                      <div className="bg-gradient-to-br from-green-50 to-teal-50 rounded-lg p-6 border border-green-200">
                        <h3 className="font-bold text-lg text-green-800 mb-3">Moon Sign (Rashi)</h3>
                        <p className="text-2xl font-bold text-success mb-2">
                          {data?.nakshatra_details?.moon_sign || data?.planets?.Moon?.sign || 'Calculating...'}
                        </p>
                        <p className="text-sm text-green-700">Your emotional nature and instincts</p>
                      </div>
                    </div>
                    
                    <div className="space-y-6">
                      <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-6 border border-orange-200">
                        <h3 className="font-bold text-lg text-orange-800 mb-3">Sun Sign</h3>
                        <p className="text-2xl font-bold text-deep-accent mb-2">
                          {data?.nakshatra_details?.sun_sign || data?.planets?.Sun?.sign || 'Calculating...'}
                        </p>
                        <p className="text-sm text-orange-700">Your core personality and ego</p>
                      </div>
                      
                      {data?.planets?.Moon && (
                        <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-6 border border-purple-200">
                          <h3 className="font-bold text-lg text-purple-800 mb-3">Moon Position</h3>
                          <p className="text-xl font-bold text-accent mb-1">
                            {data.planets.Moon.degrees_in_sign?.toFixed(2)}° in {data.planets.Moon.sign}
                          </p>
                          <p className="text-sm text-purple-700 mb-2">House: {data.planets.Moon.house}</p>
                          <p className="text-xs text-purple-600">Longitude: {data.planets.Moon.longitude?.toFixed(3)}°</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {activeTab === 'panchang' && (
                <div className="bg-surface rounded-xl p-6 border border-line">
                  ...
                  <p className="text-text-secondary mb-6">Five essential elements calculated using PyJHora for your birth time</p>
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-red-50 to-pink-50 rounded-lg p-6 border border-red-200">
                      ...
                      <p className="text-xl font-bold text-primary mb-1">
                        {(data as any)?.nakshatra_details?.tithi || (data as any)?.panchang?.tithi?.name || 'Calculating...'}
                      </p>
                      <p className="text-sm text-red-700">Lunar day</p>
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
                      ...
                      <p className="text-xl font-bold text-primary mb-1">
                        {data?.nakshatra_details?.birth_nakshatra || (data as any)?.panchang?.nakshatra?.name || 'Calculating...'}
                      </p>
                      <p className="text-sm text-blue-700">Birth star</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-6 border border-green-200">
                      ...
                      <p className="text-xl font-bold text-primary mb-1">
                        {(data as any)?.nakshatra_details?.yoga || (data as any)?.panchang?.yoga?.name || 'Calculating...'}
                      </p>
                      <p className="text-sm text-green-700">Planetary combination</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg p-6 border border-yellow-200">
                      ...
                      <p className="text-xl font-bold text-primary mb-1">
                        {(data as any)?.nakshatra_details?.karana || (data as any)?.panchang?.karana?.name || 'Calculating...'}
                      </p>
                      <p className="text-sm text-yellow-700">Half tithi</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-lg p-6 border border-purple-200">
                      ...
                      <p className="text-xl font-bold text-primary mb-1">
                        {(data as any)?.nakshatra_details?.vara || (data as any)?.panchang?.vara?.name || 'Calculating...'}
                      </p>
                      <p className="text-sm text-purple-700">Weekday</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-lg p-6 border border-teal-200">
                      ...
                      <div className="space-y-2">
                        <div>
                          <p className="text-sm font-medium text-teal-700">Ritu (Season):</p>
                          <p className="text-lg font-bold text-primary">
                            {(data as any)?.nakshatra_details?.ritu || (data as any)?.panchang?.ritu || 'Unknown'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-teal-700">Masa (Month):</p>
                          <p className="text-lg font-bold text-primary">
                            {(data as any)?.nakshatra_details?.masa || (data as any)?.panchang?.masa || 'Unknown'}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-8 p-4 bg-primary/5 rounded-lg border border-primary/20">
                    <h4 className="font-semibold text-primary mb-2">About Panchang</h4>
                    <p className="text-sm text-text-secondary leading-relaxed">
                      Panchang consists of five essential elements used in Vedic astrology: Tithi (lunar day), Nakshatra (constellation), 
                      Yoga (planetary combination), Karana (half-tithi), and Vara (weekday). These elements together provide insights 
                      into the cosmic influences at the time of birth and are crucial for muhurat selection and astrological analysis.
                    </p>
                  </div>
                </div>
              )}
          ...
          <section className="mb-12">
            <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-2xl p-8 border border-line text-center">
              <h2 className="text-2xl font-bold text-primary mb-4">Want Detailed Analysis?</h2>
              <p className="text-text-secondary mb-6 max-w-2xl mx-auto">
                This is a basic computer-generated analysis. For personalized predictions, 
                remedies, and detailed insights, book a consultation with Acharya Hari Om Shastri.
              </p>
